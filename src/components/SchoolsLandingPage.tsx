...
import { ArrowRight, Globe, Loader2, Check, Star, Users, Phone, ArrowLeft } from 'lucide-react';
...
export const SchoolsLandingPage: React.FC<LandingPageProps> = ({ onNavigate }) => {
    ...
    try {
      const response = await fetch(FORM_SUBMIT_URL, {
        ...
        body: JSON.stringify({
          website: url,
          email: email,
          _subject: `Grovia Labs Audit Request (Schools Page): ${url}`
        })
      });
      if (response.ok) {
        setIsSuccess(true);
  ...
  return (
    <div className="w-full min-h-screen bg-black relative z-10 px-6 py-12 flex flex-col justify-between overflow-hidden">
      ...
      <main className="max-w-4xl mx-auto w-full flex-1 flex flex-col items-center justify-center text-center relative z-10 mb-20">
        ...
        >
          Admissions funnels that capture <span className="italic text-white/60">parent trust.</span>
        </motion.h1>
        ...
        >
          Private schools and academies don't need static brochures; they need parent engagement. We build responsive, trust-heavy websites featuring virtual tour bookings, transparent fee sections, and seamless admissions forms.
        </motion.p>
          ...
          {isSuccess ? (
            <div className="liquid-glass rounded-2xl py-5 px-6 text-center border border-white/10 shadow-lg animate-fade-in">
              <span className="text-emerald-400 font-medium text-sm md:text-base py-2 px-2 flex items-center justify-center gap-2">
                ✓ Admissions Pipeline Audit requested! We will email you in 24 hours.
              </span>
            </div>
          ) : isError ? (
            ...
            <form onSubmit={handleAuditSubmit} className="flex flex-col sm:flex-row gap-4 p-2 bg-[#0C0C0C]/80 backdrop-blur-md rounded-2xl sm:rounded-full border border-white/5 shadow-2xl w-full">
              <div className="flex-1 flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-2 px-1">
                <div className={`flex-1 flex items-center gap-2 px-3 py-2 rounded-xl sm:rounded-full border transition-all duration-300 ${isFocusedUrl ? 'border-white/20 bg-white/[0.02]' : 'border-transparent bg-transparent'}`}>
                  ...
                  <input
                    ...
                    placeholder="Current School Site URL"
                ...
                <div className={`flex-1 flex items-center gap-2 px-3 py-2 rounded-xl sm:rounded-full border transition-all duration-300 ${isFocusedEmail ? 'border-white/20 bg-white/[0.02]' : 'border-transparent bg-transparent'}`}>
                  <Phone className={`w-4 h-4 transition-colors duration-300 ${isFocusedEmail ? 'text-white/70' : 'text-white/30'}`} />
                  <input
                    type="email"
                    required
                    value={email}
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    <span>Get Admissions Audit</span>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span>Increase campus tour registrations</span>
          </div>
          <div className="flex items-center gap-2">
            <Check className="w-4 h-4 text-emerald-400/80" />
            <span>Mobile-optimized virtual inquiries</span>
          </div>
          <div className="flex items-center gap-2">
            <Check className="w-4 h-4 text-emerald-400/80" />
            <span>LocalBusiness schema for Google Maps</span>
          </div>
        </motion.div>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-3 gap-6 w-full mt-20 sm:mt-28">
          <div className="liquid-glass border border-white/10 rounded-3xl p-8 flex flex-col items-center justify-center">
            <Users className="w-8 h-8 text-emerald-400/80 mb-4" />
            <h3 className="text-3xl font-semibold text-white mb-2">180+</h3>
            <p className="text-xs text-white/60">Enrollment inquiries generated</p>
          </div>
          <div className="liquid-glass border border-white/10 rounded-3xl p-8 flex flex-col items-center justify-center">
            <Star className="w-8 h-8 text-yellow-400/80 mb-4" />
            <h3 className="text-3xl font-semibold text-white mb-2">Top 3</h3>
            <p className="text-xs text-white/60">GMB local search map placement</p>
          </div>
          <div className="liquid-glass border border-white/10 rounded-3xl p-8 flex flex-col items-center justify-center">
            <Check className="w-8 h-8 text-blue-400/80 mb-4" />
            <h3 className="text-3xl font-semibold text-white mb-2">100%</h3>
            <p className="text-xs text-white/60">Parent inquiry form automation</p>
          </div>
        </section>
      </main>
      ...
      <footer className="max-w-5xl mx-auto w-full text-center relative z-20 border-t border-white/5 pt-6 text-white/40 text-xs">
        <p>&copy; {new Date().getFullYear()} Grovia Labs. Engineered for School Enrollments.</p>
      </footer>
    </div>
  );
};
